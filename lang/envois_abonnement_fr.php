<?php
// This is a SPIP language file  --  Ceci est un fichier langue de SPIP

if (!defined('_ECRIRE_INC_VERSION')) {
	return;
}


$GLOBALS[$GLOBALS['idx_lang']] = array(
	// B
	'bouton_afficher_abonnes' => "Afficher la liste des abonnés de ce numéro",
	'bouton_exporter_selection' => "Exporter la sélection",
	'bouton_exporter_tout' => "Exporter la totalité de la liste",
	
	// C
	'champ_selection_numero_reference_label' => "Sélectionnez un numéro de référence",
	
	// I
	'info_filtre_tous' => 'Tous',
	'info_filtre_echeance_egale' => 'Échéance ',
	'info_filtre_echeance_hors' => 'Hors échéance ',
	'info_abonne_aucun' => "Aucun abonné",
	'info_filtre_renouvellement_avec' => "Avec renouvellement",
	'info_filtre_renouvellement_sans' => "Sans renouvellement",
	
	// T
	'titre_envois_abonnements' => "Envois d'abonnements",
);
